...
namespace AutoMapper.AspNet.OData.Visitors
{
    ...
    internal sealed class MethodInserter : ExpressionVisitor
    {
        private readonly Type elementType;
        private readonly LambdaExpression lambdaExpression;

        private MethodInserter(Type elementType, LambdaExpression lambdaExpression)
        {
        ...
        public static Expression Insert(Type elementType, LambdaExpression lambdaExpression, MemberAssignment binding) =>
            new MethodInserter(elementType, lambdaExpression).Visit(binding.Expression);
        ...
        protected override Expression VisitMethodCall(MethodCallExpression node)
        {
            ...
            if (node.Method.Name.Equals(nameof(Enumerable.Select)) && elementType == this.elementType)
            {
                return Expression.Call
                (
                    LinqMethods.EnumerableOrderByMethod.MakeGenericMethod(elementType, this.lambdaExpression.Body.Type),
                    node,
